...
The current suite passes 15 unit tests and 30 Chrome browser tests. It checks a real CSV/JSON download, offline fonts/sample/icons, storage-denial/corruption handling, keyboard focus, simulated share cancellation and genuine service-worker updates. Update checks cover the background prompt and Settings button, loading a changed page, unchanged data with a running session, current/offline/failed checks, retry, and applying a deferred download offline. The share test mocks the OS API; it does not claim to verify iCloud delivery.
...
Direction 1C's smallest chip variant is used for the favicon and, following the September 24 clarification, the home-screen icons. The Apple touch PNG (180px), manifest PNGs (192/512px) and maskable PNG (512px) are generated from the same SVG with an opaque steel background; the OS applies its corner mask. Fresh filenames replace the old X icon. Browser checks verify metadata paths, decoded dimensions, chip colors, opaque pixels, the maskable safe area, and offline loading of every favicon/install icon under both root and Pages paths. Native Add to Home Screen confirmation remains an iPhone manual check. The fonts' original OFL notices ship with the app.

The subsequent iPhone readability request increases body text to 17px, primary rows/controls to 17–18px, help text/labels to 16px, and the former 9–11px captions to 14px. Secondary text is darker, and type sizes are separated from spacing dimensions. Wide stake tables have a keyboard-accessible horizontal scroll area instead of reducing their type. Checks cover 390×844 and 320×568 in addition to 402×874, including the small-screen session setup, venue picker, cash-out, detail and import flow. All screens retain the chosen original font families and weights. These size changes intentionally supersede the reference's smaller type.

The September 24 active-tab design update adds a 2px accent bar inset 22% from either side, a 2.5 icon stroke and a 700-weight label. Inactive tabs use neutral-600, a 1.5 stroke and 500-weight labels. All four icons are 22px, centered in transparent 44×30px boxes. The current tab keeps `aria-current="page"`; the New/Live action and selection behavior on detail/session screens remain unchanged. Home, Log, Stats and Settings were checked and visually inspected at 402×874 and 320×568, including a second accent, the current-page state, touch-target size, full label fit and unchanged saved data. The original Barlow Condensed family and readable 14px labels remain; actual 500/700 faces are bundled and verified offline.

The later colour refinement uses the design's darker `--color-accent-800` for the active icon and label. This token stays on the steel ramp even with another accent selected; the indicator bar continues to use the chosen accent. The design also restores the original typography, which the app already uses.

Stakes badges in Home's Recent list and the session log grow with their text and padding instead of staying 40px wide. They retain 16px type and the square outline; unusually long pairs wrap after the slash, with a final wrap fallback for an individual long number. A 30% row-width cap leaves space for the venue and results. Browser checks cover full text, padding, neighboring text separation, opening session details and unchanged records at 402px, 390px and 320px, using integer, fractional and very large synthetic stakes. Home and Log were also visually inspected at 390px and 320px.
...
- Fonts, icons and synthetic sample data are cached with the app. Updates are offered through an explicit reload and a Settings → Update app button that checks, downloads and reloads in one action. Current, offline and failure states are visible; a deferred update remains installable from Settings. An unresolved local save error prevents reload. The saved data key does not change.
