...
export async function createBackup(state, createdAt = Date.now()) {
  ...
  return { envelope, text: JSON.stringify(envelope, null, 2), name: 'ledger-backup-' + new Date(createdAt).toISOString().replace(/[:.]/g, '-') + '.json' };
}

export async function parseBackup(text) {
  let parsed;
  try { parsed = JSON.parse(text); } catch { throw new Error('That file is not a readable Ledger backup. Your data has not changed.'); }
  if (parsed?.format !== 'ledger-backup') throw new Error('That file is not a Ledger backup. Your data has not changed.');
  if (parsed.version !== 1) throw new Error('This backup version is not supported. Update Ledger before restoring it.');
  if (!Number.isFinite(parsed.createdAt) || !validateSaved(parsed.ledger) || typeof parsed.checksum !== 'string' || parsed.checksum !== await digest(JSON.stringify(parsed.ledger))) {
    throw new Error('This Ledger backup is damaged or incomplete. Your data has not changed.');
  }
  return parsed;
}
