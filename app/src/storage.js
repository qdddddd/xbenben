import { DEFAULT_SETTINGS, ACCENTS, setupFrom, latestSession, playType, validHandsPerHour } from './domain.js';
...
const validPlayType = value => value === undefined || ['live', 'online'].includes(value);
const validDraft = d => d && text(d.venue) && text(d.city) && text(d.game) && amount(d.sb) && amount(d.bb) && d.bb > 0 &&
  Number.isInteger(d.seats) && d.seats > 0 && d.seats <= 100 && text(d.buyIn) && /^\d{0,9}$/.test(d.buyIn) && (d.cur == null || (text(d.cur) && d.cur.length > 0)) && validPlayType(d.playType);
export function validSession(s, live = false) {
  return s && text(s.id) && text(s.cur) && text(s.venue) && text(s.city) && text(s.game) && text(s.notes) &&
    ...
    Array.isArray(s.buyIns) && s.buyIns.length > 0 && s.buyIns.every(b => amount(b.amount) && Number.isFinite(b.at)) && validPlayType(s.playType);
}
export function validateSaved(data) {
  ...
  return data?.version === 1 && (data.lastBackupAt == null || Number.isFinite(data.lastBackupAt)) && Array.isArray(data.sessions) && data.sessions.every(x => validSession(x)) &&
    ...
    ['liveHandsPerHour', 'onlineHandsPerHour'].every(key => s[key] === undefined || validHandsPerHour(s[key])) &&
...
export function freshLedger() {
  return { sessions: [], active: null, settings: { ...DEFAULT_SETTINGS },
    draft: { venue: 'Bellagio', city: 'Las Vegas', cur: 'USD', sb: 2, bb: 5, game: 'NLHE', seats: 9, playType: 'live', buyIn: '' },
...
export function normalizeSaved(data) {
  ...
  const lastSetup = data.lastSetup === undefined ? (last ? setupFrom(last) : null) : data.lastSetup;
  return { ...Object.fromEntries(persistedKeys.map(k => [k, data[k]])),
    settings: { ...DEFAULT_SETTINGS, ...data.settings },
    draft: { ...data.draft, cur: data.draft.cur || data.active?.cur || data.settings.currency, playType: playType(data.draft) },
    lastSetup: lastSetup ? { ...lastSetup, playType: playType(lastSetup) } : null,
