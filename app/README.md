...
The active bottom tab has a 2px accent bar, a thicker icon stroke and a bold label, alongside its `aria-current` state. Its icon and label use the design's darker steel blue (`--color-accent-800`); the bar follows the selected accent colour. All four tab icons are 22px inside unfilled 44×30px boxes. Inactive labels use medium weight; both medium and bold Barlow Condensed fonts are bundled for offline use. The New/Live action keeps its existing blueprint button.
